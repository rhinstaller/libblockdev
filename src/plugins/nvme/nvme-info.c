...
BDNVMEControllerInfo * bd_nvme_get_controller_info (const gchar *device, GError **error) {
    ...
    if (ret == -1) {
        ...
        g_set_error (error, BD_NVME_ERROR, BD_NVME_ERROR_FAILED,
                     "NVMe Identify Controller command call failed: %s", strerror_l (errno, c_locale));
        close (fd);
        return NULL;
    }
    close (fd);
    ...
    info->feat_multiport = (ctrl_id.cmic & NVME_CTRL_CMIC_MULTI_PORT) == NVME_CTRL_CMIC_MULTI_PORT;
    info->feat_multictrl = (ctrl_id.cmic & NVME_CTRL_CMIC_MULTI_CTRL) == NVME_CTRL_CMIC_MULTI_CTRL;
    info->feat_sriov = (ctrl_id.cmic & NVME_CTRL_CMIC_MULTI_SRIOV) == NVME_CTRL_CMIC_MULTI_SRIOV;
    info->feat_ana_supported = (ctrl_id.cmic & NVME_CTRL_CMIC_MULTI_ANA_REPORTING) == NVME_CTRL_CMIC_MULTI_ANA_REPORTING;
    ...
    info->feat_format = (ctrl_id.oacs & NVME_CTRL_OACS_FORMAT) == NVME_CTRL_OACS_FORMAT;
    info->feat_ns_mgmt = (ctrl_id.oacs & NVME_CTRL_OACS_NS_MGMT) == NVME_CTRL_OACS_NS_MGMT;
    info->feat_selftest = (ctrl_id.oacs & NVME_CTRL_OACS_SELF_TEST) == NVME_CTRL_OACS_SELF_TEST;
    ...
    info->feat_one_selftest = (ctrl_id.dsto & NVME_CTRL_DSTO_ONE_DST) == NVME_CTRL_DSTO_ONE_DST;
    info->feat_sanitize_crypto = (ctrl_id.sanicap & NVME_CTRL_SANICAP_CES) == NVME_CTRL_SANICAP_CES;
    info->feat_sanitize_block = (ctrl_id.sanicap & NVME_CTRL_SANICAP_BES) == NVME_CTRL_SANICAP_BES;
    info->feat_sanitize_overw = (ctrl_id.sanicap & NVME_CTRL_SANICAP_OWS) == NVME_CTRL_SANICAP_OWS;
    ...
    info->feat_format_all_ns = (ctrl_id.fna & NVME_CTRL_FNA_FMT_ALL_NAMESPACES) == NVME_CTRL_FNA_FMT_ALL_NAMESPACES;
    info->feat_secure_erase_all = (ctrl_id.fna & NVME_CTRL_FNA_SEC_ALL_NAMESPACES) == NVME_CTRL_FNA_SEC_ALL_NAMESPACES;
    info->feat_secure_erase_crypto = (ctrl_id.fna & NVME_CTRL_FNA_CRYPTO_ERASE) == NVME_CTRL_FNA_CRYPTO_ERASE;
...
BDNVMENamespaceInfo *bd_nvme_get_namespace_info (const gchar *device, GError **error) {
    ...
    if (ret != 0) {
        g_set_error (error, BD_NVME_ERROR, BD_NVME_ERROR_FAILED,
                     "Error getting Namespace Identifier (NSID): %s", strerror_l (-ret, c_locale));
        close (fd);
        return NULL;
    }
    ...
    if (ret == -1) {
        /* generic errno errors */
        g_set_error (error, BD_NVME_ERROR, BD_NVME_ERROR_FAILED,
                     "NVMe Identify Namespace command call failed: %s", strerror_l (errno, c_locale));
        close (fd);
        return NULL;
    }
    close (fd);
    ...
    info->feat_thin = (ns_info.nsfeat & NVME_NS_FEAT_THIN) == NVME_NS_FEAT_THIN;
    info->feat_multipath_shared = (ns_info.nmic & NVME_NS_NMIC_SHARED) == NVME_NS_NMIC_SHARED;
    info->feat_format_progress = (ns_info.fpi & NVME_NS_FPI_SUPPORTED) == NVME_NS_FPI_SUPPORTED;
...
BDNVMESmartLog * bd_nvme_get_smart_log (const gchar *device, GError **error) {
    ...
    if (ret == -1) {
        /* generic errno errors */
        g_set_error (error, BD_NVME_ERROR, BD_NVME_ERROR_FAILED,
                     "NVMe Get Log Page - SMART / Health Information Log command call failed: %s", strerror_l (errno, c_locale));
        close (fd);
        return NULL;
    }
    close (fd);
    ...
    log->warning_crit_spare = (smart_log.critical_warning & NVME_SMART_CRIT_SPARE) == NVME_SMART_CRIT_SPARE;
    log->warning_crit_temp = (smart_log.critical_warning & NVME_SMART_CRIT_TEMPERATURE) == NVME_SMART_CRIT_TEMPERATURE;
    log->warning_crit_degraded = (smart_log.critical_warning & NVME_SMART_CRIT_DEGRADED) == NVME_SMART_CRIT_DEGRADED;
    log->warning_crit_ro = (smart_log.critical_warning & NVME_SMART_CRIT_MEDIA) == NVME_SMART_CRIT_MEDIA;
    log->warning_crit_volatile_mem = (smart_log.critical_warning & NVME_SMART_CRIT_VOLATILE_MEMORY) == NVME_SMART_CRIT_VOLATILE_MEMORY;
    log->warning_crit_pmr_ro = (smart_log.critical_warning & NVME_SMART_CRIT_PMR_RO) == NVME_SMART_CRIT_PMR_RO;
...
BDNVMEErrorLogEntry ** bd_nvme_get_error_log_entries (const gchar *device, GError **error) {
    ...
    if (ret == -1) {
        g_set_error (error, BD_NVME_ERROR, BD_NVME_ERROR_FAILED,
                     "NVMe Get Log Page - Error Information Log Entry command call failed: %s", strerror_l (errno, c_locale));
        close (fd);
        g_free (err_log);
        return NULL;
    }
    ...
    for (i = 0; i < elpe; i++) {
        if (le64_to_cpu (err_log[i].error_count) > 0) {
            ...
            _nvme_status_to_error (ret, FALSE, &entry->command_error);
